...
//   tf::Transform tabletag_table_transform;
   ...
   tf::Transform  camera_table_transform;
         ...
         if(get_tabletag_transform==0)
	 {
      ...
      if (get_tabletag_transform!=0){
          br.sendTransform(tf::StampedTransform(world_tabletag_transform*tag_table_transform, latest_detection_time, "/world", "/table"));
					camera_table_transform= optical_transform * world_camera_transform.inverse() * world_tabletag_transform*tag_table_transform * table_tabletop_transform;
          ...
          nh.setParam("camera_table_transform/position/x",camera_table_transform.getOrigin().getX ());
          nh.setParam("camera_table_transform/position/y",camera_table_transform.getOrigin().getY ());
          nh.setParam("camera_table_transform/position/z",camera_table_transform.getOrigin().getZ ());
          nh.setParam("camera_table_transform/quaternion/x",camera_table_transform.getRotation().getX ());
          nh.setParam("camera_table_transform/quaternion/y",camera_table_transform.getRotation().getY ());
          nh.setParam("camera_table_transform/quaternion/z",camera_table_transform.getRotation().getZ ());
          nh.setParam("camera_table_transform/quaternion/w",camera_table_transform.getRotation().getW ());
      }
    }
};
